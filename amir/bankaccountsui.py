...
class BankAccountsUI:
    ...
    def __init__(self, background):
        ...
        column = Gtk.TreeViewColumn(
            _('Account Name'), Gtk.CellRendererText(), text=1)
        treeview.append_column(column)
        column = Gtk.TreeViewColumn(
            _('Number'), Gtk.CellRendererText(), text=2)
        treeview.append_column(column)
        ...
        treeview.append_column(column)
        column = Gtk.TreeViewColumn(
            _('Bank Name'), Gtk.CellRendererText(), text=5)
        treeview.append_column(column)
    ...
    def on_save_clicked(self, button):
        ...
        if len(msg):
            dialog = Gtk.MessageDialog(
                None, 0, Gtk.MessageType.ERROR, Gtk.ButtonsType.OK, msg)
            dialog.run()
            dialog.destroy()
            return
        ...
        result = self.bankaccounts_class.add_account(id,
                                                     ...
                                                     bank_id,
                                                     self.builder.get_object(
                                                         'bank_branch').get_text(),
                                                     self.builder.get_object(
                                                         'bank_address').get_text(),
                                                     self.builder.get_object(
                                                         'bank_phone').get_text(),
                                                     self.builder.get_object(
