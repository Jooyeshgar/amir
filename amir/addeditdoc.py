...
class AddEditDoc:
    ...
    def __init__(self, number=0, background=None):
        ...
        column = Gtk.TreeViewColumn(
            _("Subject Code"), Gtk.CellRendererText(), text=1)
        column.set_spacing(5)
        column.set_resizable(True)
        self.treeview.append_column(column)
        column = Gtk.TreeViewColumn(
            _("Subject Name"), Gtk.CellRendererText(), text=2)
        column.set_spacing(5)
        column.set_resizable(True)
        ...
        self.treeview.append_column(column)
        column = Gtk.TreeViewColumn(
            _("Description"), Gtk.CellRendererText(), text=5)
        column.set_spacing(5)
        column.set_resizable(True)
        self.treeview.append_column(column)
        ...
        if number > 0:
            if self.cl_document.set_bill(number):
                ...
                msgbox = Gtk.MessageDialog(
                    self.window, Gtk.DialogFlags.MODAL, Gtk.MessageType.WARNING, Gtk.ButtonsType.OK_CANCEL, msg)
    ...
    def saveRow(self, code, amount, type, desc, iter=None):
        ...
        if iter != None:
            self.liststore.set(iter, 1, code, 2, sub.name,
                               3, debt, 4, credit, 5, desc)
        else:
            self.numrows += 1
            numrows = str(self.numrows)
            if config.digittype == 1:
    ...
    def updateSum(self, window):
        ...
        self.builder.get_object("difference").set_text(
            utility.LN(abs(self.credit_sum - self.debt_sum)))
