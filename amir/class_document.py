...
class Document:
    ...
    def delete(self):
        share.config.db.session.query(Notebook).filter(
            Notebook.bill_id == self.id).delete()
        ...
        if self.number > 0:
            ...
            for notbook in self.notebooks:
                if notbook[3] == 0:  # notebook id == 0    # self.id = bill.ID
                    share.config.db.session.add(
                        Notebook(notbook[0], self.id, notbook[1], notbook[2]))
                else:
            ...
            for deletes in delete_items:
                share.config.db.session.query(Notebook).filter(
                    Notebook.id == deletes).delete()
        else:
            logging.debug("class_document : function save : else_2")
            if factorId:  # editing factor ...
                logging.debug(
                    "class_document : function save: else_2 / if factorId")
                billId = share.config.db.session.query(Notebook) . filter(
                    Notebook.factorId == factorId) . first() . bill_id
                neededBill = share.config.db.session.query(
                    Bill) . filter(Bill. id == billId)
                #self.number = neededBill.first().number
                neededBill.update(
                    {Bill.lastedit_date: date.today(), Bill.date: self.date})
                share.config.db.session.commit()
                self.id = billId

                old_notebooks = share.config.db.session.query(
                    Notebook)    . filter(Notebook.bill_id == billId)  .all()
                for nb in old_notebooks:
                    share.config.db.session.delete(nb)
            else:  # adding new factor and bill
                logging.debug(
                    "class_document : function save: else_2 / else not factorId")
                query = share.config.db.session.query(Bill.number)
                # get latest bill  (order by number)
                ...
                if last != None:
                    ...
                    self.number = 1
                permanent = True if (
                    factorId == 0 or self.cheques != []) else False
                bill = Bill(self.number, self.creation_date,
                            date.today(), self.date, permanent)
            ...
            for notebook in self.notebooks:
                share.config.db.session.add(
                    Notebook(notebook[0], self.id, notebook[1], notebook[2], factorId=notebook[3]))

                # triggers in automatic accounting
            for cheque in self.cheques:
                n = Notebook(cheque[0], self.id, cheque[1],
                             cheque[2], chqId=cheque[3])
