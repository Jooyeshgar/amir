...
class TBalanceReport:
    ...
    def createReport(self):
        report_data = []
        remaining = 0
        report_header = [_("Code"), _("Ledger name"), _(
        ...
        for s in result:
            res = query1.filter(
                and_(Subject.lft >= s.lft, Subject.lft <= s.rgt, Notebook.value < 0)).first()
            if res[0] == None:
                debt_sum = 0
            else:
                debt_sum = res[0]
            ...
            report_data.append(
                (s.code, s.name, utility.LN(-debt_sum), utility.LN(credit_sum), remain))
    ...
    def createPrintJob(self):
        ...
        html = '<p ' + self.reportObj.subjectHeaderStyle + '><u>' + \
            _("Trial Balance") + '</u></p><p ' + self.reportObj.detailHeaderStyle + \
            '>' + _("Date") + ': ' + todaystr + '</p>'
